...
Simply run the following python file, changing the `image_path` to your desired image
```bash
python scripts/sample_image.py
```

(un-comment `conf.apply_low_vram_defaults()` if the GPU memory is less than 12 Gb)
...
If a folder contains all images, run
```bash
python scripts/sample_folder.py
```
